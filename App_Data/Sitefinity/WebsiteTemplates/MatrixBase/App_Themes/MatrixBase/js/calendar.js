...
module.exports = function () {
    ...
    iafcCalendarApp.controller('CalendarController', ['$scope', 'apiService', 'filterFilter', function ($scope, apiService, filterFilter) {
        ...
        $scope.init = function () {
            ...
            apiService.getFutureEvents().then(function (data) {
                ...
                $scope.events = $.map(data.events, function(event){
					event.dateObject = new Date( event.eventStart );
					event.startend = moment(event.eventStart).format('LLL') + '-' + moment(event.eventEnd).format('LLL');
                    event.eventStart = moment(event.eventStart).format('LLL');
                    event.eventEnd = moment(event.eventEnd).format('LLL');
					event.startMonthShort = $scope.monthShort(event.eventStart);
                    ...
                    event.eventSpan = $scope.prettyDate(event.eventStart);
					...
					event.topics  = event.categories.map(function(category){
					  ...
					  if($scope.prettyDate(event.eventStart) != $scope.prettyDate(event.eventEnd)){
                        if(event.startMonth != event.endMonth){
							event.eventSpan = event.startMonth + " " + event.startDay  + " - " + event.endMonth + event.endDay + ", " + $scope.year(event.eventEnd);
						}
						else{
							event.eventSpan = event.startMonth + " " + event.startDay  + " - " + event.endDay + ", " + $scope.year(event.eventEnd);
						}
                      }
					  else{
						  	event.eventSpan = $scope.prettyDate(event.eventStart);
        ...
        $scope.prettyDate = function (date) {
            return moment(date,  "LLL").format('MMMM D, YYYY');
        };
		$scope.monthShort = function (date) {
            return moment(date, 'MMMM D YYYY' ).format('MMM');
        };
        $scope.month = function (date) {
            return moment(date, 'MMMM D YYYY' ).format('MMMM');
        };
        $scope.day = function (date) {
            return moment(date, 'MMMM D YYYY').format(' D ');
        };
		$scope.year = function (date) {
			return moment(date, 'MMMM D YYYY').format('YYYY');
		}

		$scope.keywordsearch = function(event){
			var keywordString = event.title + " " + event.topics.join(" ") + " " + event.location.state + " " + event.location.city;

			if(!$scope.search.keyword){
				return true;
			}

			if($scope.search.keyword && keywordString.toLowerCase().indexOf($scope.search.keyword.toLowerCase()) > -1){
				return true;
			}
			return false;
		}

		$scope.statesearch = function(event){
			...
			if($scope.search.location && $scope.search.location.state == event.location.state){
				return true;
			}
			return false;
        ...
        $scope.$watch('categories|filter:{selected:true}', function (nv) {
            $scope.categorySelection = $.map(nv, function (filter) {
                return filter.key;
            });
        }, true);

		$scope.$watch('locations|filter:{selected:true}', function (nv) {
                $scope.locationSelection = $.map(nv, function (filter) {
                return filter.key;
            });
        }, true);
	...
	iafcCalendarApp.filter('selfPaced', function () {
        return function (events) {
            return events.filter(function (event) {

                if (event.url.indexOf("/self-paced/") == -1) {
                    return true;
                }
